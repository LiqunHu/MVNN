              ...
              <div id="toolbar">
                  <div class="form-inline" role="form">
                      <div class="form-group" style="width:200px">
                          <select class="form-control select2" multiple id="domainID"> </select>
                      </div>
...
const apiUrl = '/api/system/domainmenucontrol?method='
...
export default {
    ...
    function initPage() {
        _self.$http.post(apiUrl + 'init', {}).then((response) => {
            ...
            common.initSelect2($('#domainID'), retData['domainInfo'])
            $('#domainID').on('select2:select', function(evt) {
                getCheckData()
    ...
    function getCheckData() {
        ...
        _self.$http.post(apiUrl + 'search', {
            ...
            for (let i = 0; i < retData.domainMenu.length; i++) {
                for (let j = 0; j < menuInfo.length; j++) {
                    if (retData.domainMenu[i] === menuInfo[j].menu_id) {
...
methods: {
    modify: function(event) {
        let domainID = $('#domainID').val()
        if (!domainID) {
            common.dealPromptCommon('未选定机构，不能分配菜单')
        } else {
            let menuTableData = $('#menuTable').bootstrapTable('getSelections')
            this.$http.post(apiUrl + 'modify', {
                'domain_id': domainID[0],
                'domainMenu': menuTableData
            }).then((response) => {
                common.dealPromptCommon('操作成功')
            }, (response) => {
                // error callback
                console.log('get data error')
